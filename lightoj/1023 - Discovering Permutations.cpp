...
bool nextpermutation(string::iterator bg, string::iterator ed)
{
    string::iterator i = ed-1;
    while(i != bg && *i <= *(i-1)) i--;
    if(i == bg) return false;
    i--;
    string::iterator j = ed-1;
    while(*j <= *i) j--;
    swap(*i, *j);
    reverse(i+1, ed);
    return true;
...
//    WRITE( "out.txt" );
    ...
    while(t--) {
        ...
        } while(--k && nextpermutation(s.begin(), s.begin()+n));
    }
