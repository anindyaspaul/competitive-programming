...
vector <int> coin;
vector <int> valueA, valueB;

void backtrackA(int i, int n, int sum)
{
    if(i == n) valueA.PB(sum);
    else {
        backtrackA(i+1, n, sum+2*coin[i]);
        backtrackA(i+1, n, sum+coin[i]);
        backtrackA(i+1, n, sum);
    }
}

void backtrackB(int i, int n, int sum)
{
    if(i == n) valueB.PB(sum);
    else {
        backtrackB(i+1, n, sum+2*coin[i]);
        backtrackB(i+1, n, sum+coin[i]);
        backtrackB(i+1, n, sum);
    }
}
...
//    WRITE( "out.txt" );
    ...
    while(t--) {
        ...
        backtrackA(0, n/2, 0);
        backtrackB(n/2, n, 0);
        sort(valueB.begin(), valueB.end());

        bool done = false;
        for(int i = 0; !done && i < valueA.size(); i++) {
            if(binary_search(valueB.begin(), valueB.end(), k-valueA[i]))
                done = true;
        }
