...
public:
    ...
    bool done() {
        for(int i = 0; i < m; i++) {
            if(seat[i]->current) return false;
            for(int j = 0; j < n; j++) {
                if(seat[i]->wasHere[j] == false)
                    return false;
            }
        }
    ...
    void rotate() {
        if(seat[pos]->current) {
            Person *tmp = seat[pos]->current;
            seat[pos]->current = NULL;
            if(!tmp->done(m)) {
                que.PB(tmp);
            }
            else delete tmp;
        ...
        for(int i = 0; i < que.size(); i++) {
            if(!que[i]->satOn[pos]) {
                que[i]->satOn[pos] = true;
                seat[pos]->current = que[i];
                seat[pos]->wasHere[que[i]->id] = true;
