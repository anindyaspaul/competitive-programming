...
{
    return point[i][0]*(point[j][1]-point[k][1]) +
           point[j][0]*(point[k][1]-point[i][1]) +
           point[k][0]*(point[i][1]-point[j][1]) == 0;
}
...
int func(int mask)
{
    ...
    for(int i = 0; i < n; i++) {
        if(!checkBit(mask, i)) {
            for(int j = i; j < n; j++) {
                if(!checkBit(mask, j))
                    ret = min(ret, 1 + func(mask|cover[i][j]));
            }
            break;
        }
    }
