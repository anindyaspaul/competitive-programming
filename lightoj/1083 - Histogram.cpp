...
Node build(int root, int st, int ed)
{
    if(st == ed) {
        nd[root].val = ara[st];
        nd[root].pos = st;
    ...
    int mid = (st+ed)/2;
    Node l = build(2*root, st, mid);
    Node r = build(2*root+1, mid+1, ed);

    if(l.val < r.val) nd[root] = l;
    else nd[root] = r;

    return nd[root];
}

Node query(int root, int st, int ed, int i, int j)
{
    if(i > ed || j < st) return Node(INF, -1);

    if(i <= st && j >= ed) return nd[root];

    int mid = (st+ed)/2;
    Node l = query(2*root, st, mid, i, j);
    Node r = query(2*root+1, mid+1, ed, i, j);
...
int calc(int st, int ed)
{
    ...
    Node tmp = query(1, 0, n-1, st, ed);

    int ret = (ed-st+1)*tmp.val;
    ret = max(ret, calc(st, tmp.pos-1));
    ret = max(ret, calc(tmp.pos+1, ed));
