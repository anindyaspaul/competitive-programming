...
int calc(int i, int g)
{
    ...
    if(dp.find(make_pair(i, g)) != dp.end()) return dp[make_pair(i, g)];

    int ret = INF;
    ret = min(ret, c[i] + calc(i+1, GCD(g, l[i])));
    ret = min(ret, calc(i+1, g));

    return dp[make_pair(i, g)] = ret;
}
